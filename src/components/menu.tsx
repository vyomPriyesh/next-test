    ...
    <div className={`flex flex-row gap-1 overflow-x-auto text-nowrap heading pt-1.5 ps-1 pb-1`}>
      {first && (
        <Link
          ...
          className={`${
            active?.to === 0 ? 'text-black border-black' : 'text-red-500 border-red-500'
      ...
      {menu.map((list, i) => (
        <Link
          key={i}
          href={first ? `/?id=${list.to}` : `/${list.to}`}
          onClick={() => handleClick(list)}
          className={`${
            active?.to !== list.to ? 'text-red-500 border-red-500' : 'border-black'
          } px-3 border-2 rounded-md font-medium text-sm md:text-base`}
        >
          {list.name}
        </Link>
